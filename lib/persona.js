...
$(document).ready(function() {
	...
	$('body').click(function(){
		setTimeout(function(){
			y_o = 0;
			$('main').find('.open > .dropdown-menu').each(function(){
				if ($(this).offset().top + $(this).outerHeight() > y_o) {
					y_o = $(this).offset().top + $(this).innerHeight();
				}
			});
			if (y_o > ($('main').offset().top + $('main').outerHeight())) {
				$('main').css('padding-bottom', (50 + y_o - ($('main').offset().top + $('main').outerHeight())) + 'px');
			} else {
